...
function update() {
    ...
    // Mario physics
    if (!mario.onLadder) {
        mario.dy += mario.gravity * ((canvas.height * 0.65) / 500); // Adjust gravity for landscape, increased height, compacted content at 65%
        mario.y += mario.dy;
        if (mario.jumping) {
            mario.dy = -10 * ((canvas.height * 0.65) / 500); // Adjust jump for landscape, increased height, compacted content at 65%
            mario.jumping = false;
        }
    } else {
        mario.dy = 0;
